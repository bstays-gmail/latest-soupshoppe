...
export default function TVDisplay() {
  ...
  useEffect(() => {
    ...
    const fitText = (text: string, maxWidth: number, baseFontSize: number, fontFamily: string): number => {
    ...
    const drawCenteredText = (text: string, x: number, y: number, maxWidth: number, baseFontSize: number, fontFamily: string, color: string) => {
      const fontSize = fitText(text, maxWidth, baseFontSize, fontFamily);
      ctx.font = `bold ${fontSize}px ${fontFamily}`;
      ctx.fillStyle = color;
    ...
    drawCenteredText("Today's Specials", width / 2, 130, width - 120, 90, 'Georgia, serif', '#FFFFFF');
    ...
    drawCenteredText('Soups', width / 2, yPos + 60, width - 120, 72, 'Georgia, serif', COLORS.paprika);
    ...
    for (let i = 0; i < activeSoups.length && i < 6; i++) {
      ...
      const fontSize = fitText(displayText, width - 200, 46, 'Arial, sans-serif');
    ...
    const cardGap = 30;
    const cardContentMaxWidth = cardWidth - 80;

    drawShadowRect(40, yPos, cardWidth, cardHeight, 25, COLORS.paprikaLight);
    drawCenteredText('Hot Panini', width / 2, yPos + 70, cardContentMaxWidth, 56, 'Georgia, serif', '#FFFFFF');
    const panini = specials?.panini ? getFreshItem(specials.panini) : null;
    if (panini) {
      drawCenteredText(panini.name, width / 2, yPos + 155, cardContentMaxWidth, 46, 'Arial, sans-serif', COLORS.bisque);
    }

    yPos += cardHeight + cardGap;

    drawShadowRect(40, yPos, cardWidth, cardHeight, 25, COLORS.warmBrown);
    drawCenteredText('Cold Sandwich', width / 2, yPos + 70, cardContentMaxWidth, 56, 'Georgia, serif', '#FFFFFF');
    const sandwich = specials?.sandwich ? getFreshItem(specials.sandwich) : null;
    if (sandwich) {
      drawCenteredText(sandwich.name, width / 2, yPos + 155, cardContentMaxWidth, 46, 'Arial, sans-serif', COLORS.bisque);
    }

    yPos += cardHeight + cardGap;

    drawShadowRect(40, yPos, cardWidth, cardHeight, 25, COLORS.sage);
    drawCenteredText('Featured Salad', width / 2, yPos + 70, cardContentMaxWidth, 56, 'Georgia, serif', '#FFFFFF');
    const salad = specials?.salad ? getFreshItem(specials.salad) : null;
    if (salad) {
      drawCenteredText(salad.name, width / 2, yPos + 155, cardContentMaxWidth, 46, 'Arial, sans-serif', COLORS.darkText);
    }

    yPos += cardHeight + cardGap;

    drawShadowRect(40, yPos, cardWidth, cardHeight, 25, COLORS.bisque);
    drawCenteredText('Dinner Entrée', width / 2, yPos + 70, cardContentMaxWidth, 56, 'Georgia, serif', COLORS.paprika);
    const entree = specials?.entree ? getFreshItem(specials.entree) : null;
    if (entree) {
      drawCenteredText(entree.name, width / 2, yPos + 155, cardContentMaxWidth, 46, 'Arial, sans-serif', COLORS.darkText);
    }
