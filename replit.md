...
7. **Announcement Banner**: Admin-configurable overlay for promotions or closure notices, visitors can dismiss with X button
...
### Static Assets
- **Cloudinary CDN**: All 233 menu item images hosted on Cloudinary (cloud_name: dlcrh8uee)
- **Image URL Mapping**: `client/src/lib/cloudinary-images.ts` contains all Cloudinary URLs embedded in code
- **No Database Sync Required**: Images work automatically in dev/production without database synchronization
- Generated AI images for custom items stored in `/public/generated-images/`
- Custom fonts: Playfair Display (serif headings), Inter (body text)

### Image Architecture
- All built-in menu items have Cloudinary URLs embedded directly in `store.ts`
- Uses `getImageUrl(itemId)` helper function that maps item IDs to Cloudinary URLs
- DB_VERSION incremented to 5 to force local storage updates
- Custom items created by admins use local generated images